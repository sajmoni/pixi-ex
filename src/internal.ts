...
export const getCells = ({ width, height, numberOfCells, resolution, scale }: GetCellsArgs): Cell[] => {
  ...
  for (let x = 0; x < numberOfCells; x++) {
    for (let y = 0; y < numberOfCells; y++) {
      cells.push({ x: x * cellWidth, y: y * cellHeight, width: cellWidth, height: cellHeight })
    }
  }
